...
const getCord = async (req, res, next) => {
    ...
    const {longitude, latitude, address} = await getCorde(street, city, state )
    next({longitude, latitude, address})
}

const getDeliveryDistance = async (corde, req, res, next) => {    
    try {
        const {km, minutes} = await getDistance(corde.longitude, corde.latitude)
        return res.status(200).json({msg: "Percurso encontrado com sucesso!", results: {distance: km, duration: minutes, address: corde.address}, status: 200})
    } catch (error) {
